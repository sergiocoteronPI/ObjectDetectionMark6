        ...
        for col in range(W):
            ...
            for class_loop in range(C):
                ...
                for boxLoop in range(B):
                    ...
                    bb.x = (col + expit_c(Bbox_pred[row, col, class_loop, boxLoop, 0])) / W
                    bb.y = (row + expit_c(Bbox_pred[row, col, class_loop, boxLoop, 1])) / H
                    bb.w = np.exp(np.clip(Bbox_pred[row, col, class_loop, boxLoop, 2],-15,8)) / W
                    bb.h = np.exp(np.clip(Bbox_pred[row, col, class_loop, boxLoop, 3],-15,8)) / H
